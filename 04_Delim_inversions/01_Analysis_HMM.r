...
## Function to use to detect if we have started a SV group
inside_SV_group <- function(start_sv){
  return(start_sv != -1)
}

compatibility_check <- function(start_sv, end_sv, counter, min_sv_size, density_marker){
  x <- TRUE
  if ((end_sv - start_sv < min_sv_size) | (counter < density_marker * (end_sv - start_sv))){
    x <- FALSE
  }
  return(x == TRUE)
}

## Function to find SVs in the HMM outputs
add_first_pos <- function(list_positions){
  if (1 %!in% list_positions){
    return(c(1, list_positions))
  }else{
    return(list_positions)
  }
}

add_last_pos <- function(list_positions, df){
  if ((df %>% nrow) %!in% list_positions){
    return(c(list_positions, (df %>% nrow)))
  }else{
    return(list_positions)
  }
}

add_previous_pos <- function(list_positions){
  return(list_positions %>% 
           append(list_positions - 1))
}

filter_abnormal_values <- function(list_positions){
  list_positions <- list_positions[list_positions %!in% c(0)] %>% 
    sort
  if (length(list_positions) %%2 == 0){
    return(list_positions)
  }else{
    return(list_positions %>% 
             append(list_positions[3:length(list_positions) - 1]) %>% 
             sort)
  }
}

get_Svs <- function(df, bin_size = 1e5, density_marker = 0.6, window_width = 30, min_sv_size = 10, threads = 17){
  # Make a loop to iterate over each chromosome in the dataframe
  SVs <- data.frame()
  for (chromosome in (df$Chromosome %>% unique)){
    # First, subset the dataframe to keep only the chromosome of interest
    df_chrom <- df %>% 
      # Make some bins of the chosen size
      mutate(bin = (Position /bin_size) %>% round) %>% 
      group_by(Chromosome, bin, Group) %>%
      # Then, we count how many positions of each group we have per bin and select the maximum,
      # therefore, classifying each bin into a group
      summarize(counting = n(), .groups = "drop_last") %>%
      ...
      filter(Chromosome == chromosome,
             Group == 2) %>%
      mutate(bin_prev = Position - lag(Position, default = 0))
    
    if ((df_chrom %>% nrow) != 0){
      group_breaks <- which(df_chrom$bin_prev > window_width) %>% 
        add_previous_pos() %>% 
        add_first_pos() %>% 
        add_last_pos(df_chrom) %>% 
        filter_abnormal_values()
      svs <- data.frame(
        "Chromosome" = rep(chromosome, length(group_breaks) / 2),
        "Start" = df_chrom$Position[
          group_breaks[
            seq(1, length(group_breaks), by = 2)
          ]
        ],
        "..Start_positions.." = group_breaks[
          seq(1, length(group_breaks), by = 2)
        ],
        "Ends" = df_chrom$Position[
          group_breaks[
            seq(2, length(group_breaks), by = 2)
          ]
        ],
        "..End_positions.." = group_breaks[
          seq(2, length(group_breaks), by = 2)
        ]
      ) %>% 
        mutate(Size = Ends - Start,
               ..Nb_lines.. = ..End_positions.. - ..Start_positions..) %>% 
        filter(Size >= min_sv_size,
               ..Nb_lines.. >= density_marker * Size) %>% 
        select(!starts_with(".."))
      
      SVs <- add_table_to_df_in_iteration(SVs, svs)
    }
  }
  return(SVs)
}
