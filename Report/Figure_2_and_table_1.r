...
data2 %>%
  ...
  facet_grid2(vars(Cline), vars(Single_location),
              scales="free",
              switch = "y",
              labeller = as_labeller(c("Size" = "Shell size \n(mm)", "Color" = "Frequency of color\nbrown",
...
size_table <- data1_size %>%
  ...
  mutate(Centre_ci = paste0("[", min(Centre_2.5, Centre_97.5) %>% round(digits=1), ", ", max(Centre_97.5, Centre_2.5) %>% round(digits=1), "]"),
         ...
         "Zone abritée" = paste(Left, Left_ci),
         "Zone exposée" = paste(Right, Right_ci),
         "Centre du cline" = paste(Centre, Centre_ci),
         "Largeur du cline" = paste(Width, Width_ci)) %>% 
  ungroup %>% 
  select(Pays, "Zone abritée", "Zone exposée", "Centre du cline", "Largeur du cline") %>% 
  paged_table()
