# Libraries
require("anyLib")
anyLib(c("tidyverse", "readxl", "ggh4x", "ggforce", "ggpubr", "ggnewscale", "rmarkdown"))
...
data_AIC <- metadata %>% 
  ...
  unique %>%
# Here, we determine which is the best model according to the calculated AICs and we calculate the delta AIC with
...
# First, we merge everything together to have all the information in the same place
...
# And we do the same for the stable model in Sweden
...
# Then, we calculate the confidence intervals for each parameter of the model we want to use to
# plot the distribution of color variation along the transect
## We start by doing this with the clinal model in France
Conf_interval_cline_color_fr <- data1_color %>% 
  ...
  bow(tie(Centre_2.5, Width_2.5, Left_2.5, Right_2.5, Centre_97.5, Width_97.5, Left_97.5, Right_97.5) :=
        ...
        confint()) %>%
  as.data.frame() %>% 
# Here, we reorganise the order of the confidence interval to always have the lowest value at the 2.5th percentile and the highest
...
# And then, we do the same thing for the stable model in Sweden
...
# Finally, we make a table that we can directly print with modifications in the character chains and by merging 
