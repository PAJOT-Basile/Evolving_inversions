# Import libraries
require("anyLib")
anyLib(c("tidyverse", "adegenet", "vcfR", "readxl", "statgenGWAS", "ggforce", "ggh4x"))
...
## France
## If the relatedness between individuals has not yet been calculated, you first have to make a list of individuals that you 
## save in a text file and then execute the command lines commented underneath
...
#                                " --out ./GWAS/French_relatedness")))
relatedness_france <- read.table("./GWAS/French_relatedness.relatedness", header = TRUE) %>% 
...
#                                " --out ./GWAS/Swedish_relatedness")))
relatedness_sweden <- read.table("./GWAS/Swedish_relatedness.relatedness", header = TRUE) %>% 
  rename(Relatedness = RELATEDNESS_AJK) %>% 
  pivot_wider(names_from = INDV2, values_from = Relatedness) %>% 
  column_to_rownames("INDV1")

# The matrix calculated by vcftools is not symmetrical but triangular, so, we make it symmetrical by hand
## France
...
# The GWAS function needs to have a gData object, so we make one for each location
## France
...
GWAS_france$GWAResult$Pheno_france %>% 
    ...
    thresholds_manhattan(aes(y = logp_val, facetting = Population), values = 5, palette = c("grey71", "turquoise4")) +
...
GWAS_france$GWAResult$Pheno_france %>% 
    ...
    facet_col(vars(Population))
                ...
                ################## Isolate candidate SNPs  ##################
## France
### Size
Size_SNPs_france <- GWAS_france$GWAResult$Pheno_france %>% 
  filter(trait == "Length") %>% 
  select(snp, pValue) %>% 
  rename(Position = snp) %>% 
  mutate(log_pval = -log10(pValue),
         SNP_name = str_split_fixed(Position, "\\.", 2)[,1]) %>% 
  filter(log_pval >= 4) %>% 
  select(Position, SNP_name, log_pval) %>% 
  as.data.frame

### Color
Color_SNPs_france <- GWAS_france$GWAResult$Pheno_france %>% 
  filter(trait == "Shell_color_naive") %>% 
  select(snp, pValue) %>% 
  rename(Position = snp) %>% 
  mutate(log_pval = -log10(pValue),
...
## Sweden
### Size
Size_SNPs_sweden <- GWAS_sweden$GWAResult$Pheno_sweden %>% 
  filter(trait == "Length") %>% 
  select(snp, pValue) %>% 
  rename(Position = snp) %>% 
  mutate(log_pval = -log10(pValue),
...
### Color
Color_SNPs_sweden <- GWAS_sweden$GWAResult$Pheno_sweden %>% 
  filter(trait == "Shell_color_naive") %>% 
  select(snp, pValue) %>% 
  rename(Position = snp) %>% 
  mutate(log_pval = -log10(pValue),
...
################## Boxplot of candidate SNPs  ##################
## France
### Size
get_genotype_transect(genetic_data = data,
                      SNP_subset = Size_SNPs_france,
                      metadata = metadata %>%
                        filter(Population == "France")) %>% 
  drop_na %>% 
  left_join(metadata, by = "Sample_Name") %>% 
  select(Sample_Name, starts_with("LG"), Length) %>% 
  pivot_longer(starts_with("LG"), names_to = "Position", values_to = "Genotype") %>% 
  mutate(Position_ = Position %>% factor(levels = str_sort(Position %>% unique, numeric = TRUE)),
         Genotype = (Genotype/2) %>% factor(levels = c("0", "0.5", "1"))) %>% 
  ggplot() +
  geom_boxplot(aes(x = Genotype, y = Length)) +
  facet_wrap(facets = vars(Position_))

### Color
get_genotype_transect(genetic_data = data,
                      SNP_subset = Color_SNPs_france %>% 
                        top_n(log_pval, n=30),
                      metadata = metadata %>%
                        filter(Population == "France")) %>% 
  drop_na %>% 
  left_join(metadata, by = "Sample_Name") %>% 
  select(Sample_Name, starts_with("LG"), Shell_color_naive) %>% 
  pivot_longer(starts_with("LG"), names_to = "Position", values_to = "Genotype") %>% 
  mutate(Position_ = Position %>% factor(levels = str_sort(Position %>% unique, numeric = TRUE))) %>% 
  ggplot() +
  geom_boxplot(aes(x = Shell_color_naive, y = Genotype/2)) +
  facet_wrap(facets = vars(Position_))

## Sweden
### Size
get_genotype_transect(genetic_data = data,
                      SNP_subset = Size_SNPs_sweden,
                      metadata = metadata %>%
                        filter(Population == "Sweden")) %>% 
  drop_na %>% 
  left_join(metadata, by = "Sample_Name") %>% 
